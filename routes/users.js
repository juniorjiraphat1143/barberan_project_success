...
router.get('/webinfor',function(req,res){
  res.render('webinfor');
})


router.post('/register',function(req,res){
  const {username,password,confirm,email,date,fname,image} = req.body;
  let errors=[];
  var today = new Date();
  var members = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
  if (!username || !email || !password || !confirm || !fname  ) {
  ...
  if(errors.length >0){
    ...
    User.findOne({email : email }).then(user =>{
      ...
      else {User.register(new User({username : req.body.username , date :req.body.date , image : req.body.image , name : req.body.fname , member_since : members , permission : "member"}),req.body.password,function(err,user){
          if(err){
...
router.post("/home",function(req,res){
  ...
  console.log(dates);
  Post4selves.create(new Post4selves({name : req.user.name , date : dates , image : req.body.image , id : req.user.id , text : req.body.text , Post_by : req.body.id , userpic : req.user.image, number_like : 0 , tag : req.body.tag}),function(error,newpost){
    if(error){
      console.log(error);
    }
    else{
...
router.post("/public",function(req,res){
  ...
  Commento.create(new Commento({text : req.body.comment , date : dates , time : times , author : req.user.name , author_image : req.user.image , author_id : req.user.id , post : req.body.post}),function(error,newpost){
    if(error){
      console.log(error);
    }
    else{
...
router.get('/hospital',middleware.isLoggedIn,function(req,res){
  res.render('hospital');
})
router.get('/covid19',middleware.isLoggedIn,function(req,res){
  res.render('covid19');
})
router.get('/logout', function(req,res){
  req.logout();
